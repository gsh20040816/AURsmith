...
function BuildsView({ liveVersion }: { liveVersion: number }) {
  ...
  return <><header className="page-header compact"><div><p className="eyebrow">W04 / B03</p><h1>构建任务</h1><p className="lede">Controller 签发 JobSpec；Worker Journal 拒绝冲突和迟到 Attempt。只有基础设施失败会自动重试两次。</p></div></header>{error && <Notice kind="error">{error}</Notice>}<section className="table-panel"><div className="section-heading"><h2>任务队列</h2><button className="secondary-button" onClick={refresh}>刷新</button></div>{jobs.length === 0 ? <div className="empty-state"><span className="empty-symbol">◇</span><div><strong>没有构建任务</strong><p>订阅产生通过审计的 Revision 后，完整依赖闭包会显示在这里。</p></div></div> : <div className="table-scroll"><table><thead><tr><th>任务</th><th>阶段</th><th>状态 / Attempt</th><th>Worker</th><th>Revision</th><th>更新时间</th></tr></thead><tbody>{jobs.map((job) => <tr key={job.id}><td><code>{job.id.slice(0, 8)}</code></td><td>{job.kind}</td><td><span className={`state ${job.status}`}>{job.failure_code ?? job.status}</span><small className="cell-note">{job.attempt_count} 次{job.next_attempt_at ? ` · ${new Date(job.next_attempt_at).toLocaleTimeString("zh-CN")} 重试` : ""}</small></td><td>{job.worker_name ?? "—"}</td><td><code>{job.revision_sha256?.slice(0, 12) ?? "—"}</code></td><td>{new Date(job.updated_at).toLocaleString("zh-CN")}</td></tr>)}</tbody></table></div>}</section></>;
}
