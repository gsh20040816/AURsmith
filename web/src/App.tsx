...
function ReleasesView() {
  ...
  const retry = async (release: Release) => {
    setBusy(release.id); setError("");
    try { await api.retryRelease(release.id); refresh(); } catch (reason) { setError(messageOf(reason)); } finally { setBusy(""); }
  };
  return <>
