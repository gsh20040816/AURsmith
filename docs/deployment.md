...
Builder secret 文件至少包括 Controller Bearer token、Publisher write-only SSH key 和 known_hosts。AURsmith 容器不开放入站端口。Builder jobs 路径必须是宿主绝对路径，Docker Socket 只挂载给 Builder daemon，不进入临时 Build 容器。
