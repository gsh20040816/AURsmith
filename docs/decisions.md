...
- ADR-054：Guest 对所有 makepkg 子进程设置 `DOTNET_CLI_USE_MSBUILD_SERVER=0` 和 `MSBUILDUSESERVER=0`。KVM Job 是一次性环境，不从长驻 MSBuild Server 获得复用收益；禁用它可避免 PKGBUILD 执行 `dotnet build-server shutdown` 后仍等待后台进程。该环境变量不改变源码、目标框架或依赖解析，只改变 .NET 构建进程生命周期。
