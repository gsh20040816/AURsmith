...
Guest Profile 是安装了 `base`、`linux`、`base-devel`、`devtools` 和 `namcap` 的通用 Arch Linux 构建机，并由动态 Profile 额外预装高频官方构建依赖。标准 systemd 作为 PID 1，负责设备初始化、挂载和孤儿进程回收；Guest Agent 只是一个一次性 systemd service，不实现 init。Guest Agent 从内核命令行读取 Controller 公钥并再次验证只读输入中的 JobSpec Envelope。Fetch Guest 启用 virtio 网卡和固定代理；Build Guest 根据 Builder 的 `AURSMITH_BUILD_NETWORK` 使用 `-nic none` 或 QEMU user networking 直接访问公网。Fetch 在下载源码和进入 Agent 审计前先调用官方 `makepkg --printsrcinfo`，将其规范化输出与 AUR commit 中的 `.SRCINFO` 比较；不一致时确定性阻断，避免基于过时 AUR RPC 元数据审计或构建。随后任务执行 `makepkg --verifysource` 和官方依赖下载，pacman 必须使用 Profile 中已初始化的 Arch keyring 验签，包身份从已验签归档的 `.PKGINFO` 提取；下载包、分离签名、版本和摘要都进入 Source Manifest。Build 任务不注入代理，以普通 `builder` 用户运行 `makepkg --cleanbuild`，并把实际网络模式写入 provenance。Controller 把完整 split outputs 和当前包的 `check()` 策略冻结进签名 JobSpec；Guest 要求实际 `.PKGINFO` 包名集合精确相等。默认执行 `check()`，只有 UI 中按包显式禁用才加入 `--nocheck`，结果同时写入 provenance。构建产物随后由 Guest 使用固定 argv 执行 namcap；namcap 的输出摘要和退出码进入 provenance，但其代表检查告警的非零状态不会推翻已经成功的 makepkg，真正阻断仍由 Publisher 的结构化复验决定。Publisher 对大型 ELF 流式提取到 Artifact 所在的受控 staging 文件系统后调用 `readelf`，不受容器小型 `/tmp` tmpfs 限制，也不把大型文件读入内存。输入中的特殊文件和越界符号链接会被拒绝。Guest Agent 生成结果并同步输出卷后关机；失败时只写结构化错误，不尝试降级为宿主构建。
