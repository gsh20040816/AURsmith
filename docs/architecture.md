...
Build 使用安装 `base-devel`、Git、GnuPG 和证书的通用 Arch Linux 容器，直接访问公网；不为具体语言、工具链或 AUR 包注入环境变量、编译参数和补丁。Docker 标准 init 作为 PID 1 处理信号与孤儿进程，Guest Agent 只是一次性子进程，不实现 init。Guest Agent 再次验证只读输入中的 JobSpec Envelope，再以普通 `builder` 用户运行 `makepkg --cleanbuild --syncdeps`；全局配置只在 Arch 默认值上关闭自动 debug 包，避免产生未声明的发布输出。首次发布完全使用 PKGBUILD 自己的 pkgrel；只有同上游版本的本地重建才在工作副本生成 `.1`、`.2` 子版本。`makepkg` 成功且产生预期 split outputs 即视为构建成功，不再执行 namcap 或 Publisher 内容复检。失败时只写结构化错误，不尝试降级为宿主构建。

JobSpec 同时固定直接运行、构建和检查依赖及其来源。AUR 依赖不会伪装成官方依赖下载。按 DAG 构建时，Builder 从同批次已成功 Build Attempt 中重新验证并复制直接 AUR 依赖产物。Build 容器先用 pacman 安装已固定的 Arch 与 AUR 依赖，再降权执行 makepkg；构建脚本可以正常联网获取其声明的其他上游依赖，不设置语言或生态白名单。BuildResult 记录固定 Build 镜像版本。
